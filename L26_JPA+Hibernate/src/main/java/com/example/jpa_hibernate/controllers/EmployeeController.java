...
//    MyJPARepository myJPARepository;
    ...
    public Employee deleteEmployee(@RequestParam("id") Integer employeeId){
    ...
    public Employee updateEmployee2(@PathVariable("id") Integer id, @RequestBody Employee employee){
        return employeeService.updateEmployeePartial(id, employee);
    }

    @GetMapping("/employees/age")
    public List<Employee> getEmployeesOlderThan(@RequestParam("age") int age){
        return employeeService.getEmployeesOlderThan(age);
    }

    @GetMapping("/employees-by-filter")
    public List<Employee> getEmployeesOlderThan(@RequestParam("department") String dept,
                                                @RequestParam("salary") Long salary){
        return employeeService.getEmployeesWithDeptAndSalary(dept, salary);
    }

    @GetMapping("/employees-by-age")
    public List<Employee> getEmployeesByAge(@RequestParam("start") Integer start,
